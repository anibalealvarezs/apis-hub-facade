export function dashboardView(config = {}) {
    return {
        ...
        dashboardDefaults: config.dashboardDefaults || {
            date_start: "",
            date_end: "",
            zero_handling: "remove",
            show_asset_group_selector: false,
        },
        dashboardOverrides: config.dashboardOverrides || {
            date_start: "",
            date_end: "",
            zero_handling: "remove",
        },
                ...
                const tryInit = () => {
                    if (typeof GridStack !== "undefined") {
                        ...
                        const updateEmptyLinePositions = () => {
                            if (!grid) return;
                            const emptyLines = document.querySelectorAll(
                            ...
                            emptyLines.forEach((el) => {
                                ...
                                const precedingWidgets = Array.from(
                                    document.querySelectorAll(
                                        "#view-grid-stack .grid-stack-item",
                                    ),
                                ).filter((wEl) => {
        ...
        _applyAssetGroupAfterInitialRender() {
            const attempt = () => {
                const settled =
                    this._pendingRenders <= 0 &&
                    this._startedCount >= this.totalCount;
                if (settled) {
        ...
        isViewAssetInGroup(channel, assetId) {
            if (!this.selectedAssetGroup || !this.channelAssetGroupMap[channel])
                return true;
            const groupAssets =
                this.channelAssetGroupMap[channel][this.selectedAssetGroup];
        ...
        applyAssetGroup() {
            if (this.openSettings) {
                for (const vKey in this.settingsSeriesOptions) {
                    ...
                    const groupAllowed =
                        groupId && this.channelAssetGroupMap[channel]?.[groupId]
                            ? this.channelAssetGroupMap[channel][groupId].map(
                                  String,
                              )
                            : [];

                    const seriesOptionsMap =
                        this.settingsSeriesOptions[vKey]?.options || {};
                    const allAvailableOptions =
                        Object.keys(seriesOptionsMap).map(String);
                    const allowedWhitelist =
                        this.settingsControls.series_allowed_assets &&
                        this.settingsControls.series_allowed_assets[vKey] &&
                        this.settingsControls.series_allowed_assets[vKey]
                            .length > 0
                            ? this.settingsControls.series_allowed_assets[
                                  vKey
                            ...
                            : this.settingsOriginalControls
                                ...
                                this.settingsOriginalControls
                                    .series_allowed_assets[vKey] &&
                                this.settingsOriginalControls
                                    .series_allowed_assets[vKey].length > 0
                              ? this.settingsOriginalControls.series_allowed_assets[
                                    vKey
                                ].map(String)
                    ...
                    if (!groupId) {
                        validAssets =
                            this.settingsOriginalControls.series_assets &&
                            this.settingsOriginalControls.series_assets[vKey]
                                ? this.settingsOriginalControls.series_assets[
                                      vKey
                                  ].map(String)
                        ...
                        const currentSelection =
                            ...
                            this.settingsControls.series_assets[vKey]
                                ? this.settingsControls.series_assets[vKey].map(
                                      String,
                                  )
                                : this.settingsOriginalControls.series_assets &&
                                    this.settingsOriginalControls.series_assets[
                        ...
                        const intersectedCurrent = currentSelection.filter(
                            (a) => groupAllowed.includes(String(a)),
                        );
                        if (intersectedCurrent.length > 0) {
                            validAssets = intersectedCurrent;
                        } else if (
                            allowedWhitelist &&
                            allowedWhitelist.length > 0
                        ) {
                            validAssets = allowedWhitelist.filter((a) =>
                                groupAllowed.includes(String(a)),
                            );
                        }
                        if (
                            validAssets.length === 0 &&
                            groupAllowed.length > 0
                        ) {
        ...
        _applyGroupToWidgetControls(controls, el) {
            ...
            if (headerEl) {
                try {
                    variables = JSON.parse(
                        headerEl.getAttribute("data-variables") || "{}",
                    );
                } catch (e) {}
                try {
                    seriesAssetsOptions = JSON.parse(
                        headerEl.getAttribute("data-series-options") || "{}",
                    );
                } catch (e) {}
                try {
                    initialControls = JSON.parse(
                        headerEl.getAttribute("data-controls") || "{}",
                    );
                } catch (e) {}
            }
            ...
            for (const vKey in variables || {}) {
                ...
                const seriesOptionsMap =
                    seriesAssetsOptions[vKey]?.options || {};
                const allAvailableOptions =
                    Object.keys(seriesOptionsMap).map(String);
                const allowedWhitelist =
                    controls.series_allowed_assets &&
                    controls.series_allowed_assets[vKey] &&
                    controls.series_allowed_assets[vKey].length > 0
                        ? controls.series_allowed_assets[vKey].map(String)
                        : initialControls.series_allowed_assets &&
                            initialControls.series_allowed_assets[vKey] &&
                            initialControls.series_allowed_assets[vKey].length >
                                0
                          ? initialControls.series_allowed_assets[vKey].map(
                                String,
                            )
                          : allAvailableOptions.length > 0
                            ? allAvailableOptions
                ...
                if (!groupId) {
                    ...
                    validAssets =
                        initialControls.series_assets &&
                        initialControls.series_assets[vKey]
                            ? initialControls.series_assets[vKey].map(String)
                            : nextControls.series_assets[vKey] || [];
                } else {
                    const currentSelection =
                        nextControls.series_assets &&
                        nextControls.series_assets[vKey]
                    ...
                    const intersectedCurrent = currentSelection.filter((a) =>
                        groupAllowed.includes(String(a)),
                    );
                    if (intersectedCurrent.length > 0) {
                        validAssets = intersectedCurrent;
                ...
                if (
                    JSON.stringify(newAssets) !== JSON.stringify(currentAssets)
                ) {
        ...
        renderWidget(widgetId, el, controls) {
            ...
            if (this.hasUserChangedGlobalDate) {
                if (this.dashboardOverrides.date_start)
                    effectiveControls.date_start =
                        this.dashboardOverrides.date_start;
                if (this.dashboardOverrides.date_end)
                    effectiveControls.date_end =
                        this.dashboardOverrides.date_end;
            } else {
                if (
                    !effectiveControls.date_start &&
                    this.dashboardDefaults.date_start
                )
                    effectiveControls.date_start =
                        this.dashboardDefaults.date_start;
                if (
                    !effectiveControls.date_end &&
                    this.dashboardDefaults.date_end
                )
                    effectiveControls.date_end =
                        this.dashboardDefaults.date_end;
            }
        ...
        reloadWidget(widgetId, controls) {
            ...
            const result =
                this.renderWidget(widgetId, el, controls) || Promise.resolve();
            return result;
        },
        ...
        updateSettingsSeriesScrollState() {
            ...
            this.canSettingsScrollSeriesRight =
        ...
        openWidgetSettings(
            ...
            this.settingsBuilderControls = JSON.parse(
                JSON.stringify(builderControls || {}),
            );
            this.settingsOriginalControls = JSON.parse(
                JSON.stringify(controls || {}),
            );
            this.settingsControls = controls || {};
            const widgetObj = (this.widgets || []).find(
                (w) => w.id === widgetId,
            );
            this.settingsControls.titles = {
                en:
                    this.settingsControls.titles?.en ||
                    widgetObj?.titles?.en ||
                    (typeof widgetObj?.title === "string"
                        ? widgetObj?.title
                        : ""),
                es:
                    this.settingsControls.titles?.es ||
                    widgetObj?.titles?.es ||
                    (typeof widgetObj?.title === "string"
                        ? widgetObj?.title
                        : ""),
            ...
            this.settingsControls.descriptions = {
                en:
                    this.settingsControls.descriptions?.en ||
                    widgetObj?.descriptions?.en ||
                    (typeof widgetObj?.description === "string"
                        ? widgetObj?.description
                        : ""),
                es:
                    this.settingsControls.descriptions?.es ||
                    widgetObj?.descriptions?.es ||
                    (typeof widgetObj?.description === "string"
                        ? widgetObj?.description
                        : ""),
            };
            if (!this.settingsControls.metrics)
                this.settingsControls.metrics = [];
            if (!Array.isArray(this.settingsControls.metrics)) {
                this.settingsControls.metrics = Object.values(
                    this.settingsControls.metrics,
                );
            }
            if (!this.settingsControls.series_assets)
                this.settingsControls.series_assets = {};
            if (!this.settingsControls.hasOwnProperty("remove_unknown"))
                this.settingsControls.remove_unknown = true;
            this.settingsSeriesOptions = seriesOptions || {};
            this.settingsVariables = variables || {};
            if (
                !this.settingsVariables ||
                Object.keys(this.settingsVariables).length === 0
            ) {
                this.settingsVariables = {};
                for (const key in this.settingsSeriesOptions) {
                    this.settingsVariables[key] = {
                        ...
                        channel: "",
                        channel_name:
                            this.settingsSeriesOptions[key]?.label || "",
                        metrics: {},
                        selected_metric: "",
                    };
                }
            ...
            if (
                this.settingsControls.raw_series &&
                Array.isArray(this.settingsControls.raw_series)
            ) {
                this.settingsControls.raw_series.forEach((s, idx) => {
                    const k = String(idx);
                    if (!this.settingsControls.series_metrics[k]) {
                        this.settingsControls.series_metrics[k] = Array.isArray(
                            s.metrics,
                        )
                            ? [...s.metrics]
                            : [];
            ...
            for (const vKey in this.settingsVariables) {
                ...
                if (vConfig) {
                    if (sourceType === "kpi" && vConfig.index !== undefined) {
                        ...
                        if (
                            !this.settingsControls.metrics[idx] &&
                            vConfig.metrics &&
                            Object.keys(vConfig.metrics).length > 0
                        ) {
                            this.settingsControls.metrics[idx] =
                                vConfig.selected_metric ||
                                Object.keys(vConfig.metrics)[0];
                        }
                    }
                    if (!this.settingsControls.series_dependencies[vKey]) {
                        const idx =
                            vConfig.index !== undefined
                                ? String(vConfig.index)
                                : null;
                        this.settingsControls.series_dependencies[vKey] =
                            (idx &&
                                this.settingsControls.series_dependencies[
                                    idx
                                ]) ||
                            vConfig.dependency ||
                            (vConfig.dependencies &&
                            Object.keys(vConfig.dependencies).length > 0
                                ? Object.keys(vConfig.dependencies)[0]
                                : "");
                    }
                }
            }
            ...
            if (!this.settingsControls.granularity) {
                this.settingsControls.granularity =
                    this.settingsBuilderControls.granularity || "daily";
            }
            ...
            for (const key in variables) {
                if (!this.settingsSearchQueries[key])
                    this.settingsSearchQueries[key] = "";
            }
        ...
        saveSettings() {
            ...
            if (
                this.settingsSourceType !== "kpi" &&
                this.settingsSourceType !== "derived_metric"
            ) {
                if (
                    controls.series_metrics &&
                    typeof controls.series_metrics === "object"
                ) {
                    const flat = [];
                    for (const sKey in controls.series_metrics) {
                        ...
                        if (Array.isArray(sm)) {
                            ...
                            if (
                                controls.raw_series &&
                                controls.raw_series[parseInt(sKey)]
                            ) {
                                controls.raw_series[parseInt(sKey)].metrics = [
                                    ...sm,
                                ];
                            }
                        }
            ...
            let minStart =
                this.settingsBuilderControls.date_start ||
                this.dashboardDefaults.date_start ||
                "";
            let maxEnd =
                this.settingsBuilderControls.date_end ||
                this.dashboardDefaults.date_end;
            ...
            if (dateAdjusted) {
                alert(
                    "Warning: The customized date range exceeded the allowed limits and was adjusted to comply.",
                );
            }
        ...
        settingsIsMetricSelected(seriesKey, metricKey) {
            ...
            const current =
                this.settingsControls.series_metrics[seriesKey] || [];
        ...
        settingsToggleAsset(seriesKey, assetId) {
            const mode =
                this.settingsSeriesOptions[seriesKey].mode || "multiple";
            const current =
        ...
        settingsSelectAll(seriesKey) {
            ...
            const allIds = Object.keys(
                this.settingsSeriesOptions[seriesKey].options,
            ).map(String);
            ...
            if (
                ...
                const groupAssets =
                    this.channelAssetGroupMap[channel][
                        this.selectedAssetGroup
                    ].map(String);
        ...
        ensureValidAssets(seriesKey, channel, selectedAssets) {
            ...
            const validAssets = (selectedAssets || []).filter((a) =>
                allowedAssets.includes(String(a)),
            );
        ...
        openPopOut(widgetId) {
            ...
            const headerEl = widgetEl?.querySelector(
                ".grid-stack-item-content",
            );
            const title =
                headerEl?.querySelector("h3")?.textContent?.trim() || "";
        ...
        closePopOut() {
            ...
            if (widgetId) {
                ...
                if (widgetEl)
                    this._popFromRect = widgetEl.getBoundingClientRect();
            ...
            if (card) {
                ...
                if (this._popFromRect) {
                    const cardRect = card.getBoundingClientRect();
                    const originX =
                        ((this._popFromRect.left +
                            this._popFromRect.width / 2 -
                            cardRect.left) /
                            cardRect.width) *
                        100;
                    const originY =
                        ((this._popFromRect.top +
                            this._popFromRect.height / 2 -
                            cardRect.top) /
                            cardRect.height) *
                        100;
                    card.style.transformOrigin = `${originX}% ${originY}%`;
        ...
        openModalSettings() {
            const headerEl = document.querySelector(
                `[data-widget-id="${this.popOutWidgetId}"]`,
            );
            if (!headerEl || !window.Alpine) return;
            const data = Alpine.$data(headerEl);
            window.dispatchEvent(
                new CustomEvent("open-widget-settings", {
                    detail: {
                        widgetId: this.popOutWidgetId,
                        builderControls: JSON.parse(
                            JSON.stringify(data.builderControls || {}),
                        ),
                        controls: JSON.parse(
                            JSON.stringify(data.controls || {}),
                        ),
                        seriesOptions: JSON.parse(
                            JSON.stringify(data.seriesOptions || {}),
                        ),
                        variables: JSON.parse(
    ...
    return {
        ...
        init() {
            ...
            this.isTimeGranularity = [
                ...
                "annually",
                "lifetime",
            ].includes(this.builderControls.granularity);
            ...
            if (!this.controls.series_metrics)
                this.controls.series_metrics = {};
            if (
                this.controls.raw_series &&
                Array.isArray(this.controls.raw_series)
            ) {
                this.controls.raw_series.forEach((s, idx) => {
                    const k = String(idx);
                    if (!this.controls.series_metrics[k]) {
                        this.controls.series_metrics[k] = Array.isArray(
                            s.metrics,
                        )
                            ? [...s.metrics]
                            : [];
            ...
            if (this.sourceType === "kpi") {
                ...
                for (const vKey in this.variables) {
                    ...
                    if (!this.controls.series_dependencies[vKey]) {
                        this.controls.series_dependencies[vKey] =
                            this.controls.series_dependencies[String(idx)] ||
                            vConfig?.dependency ||
                            "";
                    }
                }
            ...
            this.granularityOnTheGo =
                this.controls.hasOwnProperty("granularity") &&
                this.controls.granularity !== "__NOT_SET__";
            if (
                !this.controls.granularity ||
                this.controls.granularity === "__NOT_SET__"
            )
                this.controls.granularity = "";
            ...
            window.dispatchEvent(
                new CustomEvent("open-widget-settings", {
                    detail: {
                        ...
                        builderControls: JSON.parse(
                            JSON.stringify(this.builderControls),
                        ),
                        controls: JSON.parse(JSON.stringify(this.controls)),
                        seriesOptions: JSON.parse(
                            JSON.stringify(this.seriesOptions),
                        ),
        ...
        isSelected(seriesKey, assetId) {
            ...
            return this.controls.series_assets[seriesKey].includes(
                String(assetId),
            );
        },
        ...
        toggleAsset(seriesKey, assetId) {
            const mode =
                (this.seriesOptions[seriesKey] || {}).mode || "multiple";
        ...
        selectAll(seriesKey) {
            ...
            if (mode === "single") return;
            const allIds = Object.keys(
                this.seriesOptions[seriesKey].options,
            ).map(String);
        ...
        getActiveFilterCount() {
            ...
            for (const key in this.seriesOptions) {
                if (
                    this.controls.series_assets[key] &&
                    this.controls.series_assets[key].length > 0 &&
                    this.controls.series_assets[key].length <
                        Object.keys(this.seriesOptions[key].options).length
                ) {
                    count++;
                }
            }
        ...
        updateWidget() {
            ...
            const el = document.querySelector(
                `.grid-stack-item[gs-id="${this.widgetId}"] .widget-content`,
            );
            if (el) {
