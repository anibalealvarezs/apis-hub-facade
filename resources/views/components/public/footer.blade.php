...
<footer class="relative w-full z-10 py-8 px-6 sm:px-8 mt-auto flex flex-col items-center gap-4 text-base font-medium text-slate-500 dark:text-slate-400 select-none border-t border-slate-200/50 dark:border-slate-800/50">
    <nav aria-label="{{ __('Navigation Links') }}" class="flex items-center justify-center flex-wrap gap-y-2">
        ...
        <span class="w-1 h-1 bg-brand-blue/30 dark:bg-brand-blue/20 rounded-full" aria-hidden="true"></span>
        <a href="{{ app()->getLocale() === 'es' ? route('legal.tos.es') : route('legal.tos') }}" class="px-3 py-1.5 mx-1 sm:mx-2 hover:text-brand-blue {{ request()->routeIs('legal.tos*') ? 'text-brand-blue' : '' }} transition-colors">{{ __('Terms') }}</a>
        <span class="w-1 h-1 bg-brand-teal/30 dark:bg-brand-teal/20 rounded-full" aria-hidden="true"></span>
        <a href="{{ app()->getLocale() === 'es' ? route('legal.data-deletion.es') : route('legal.data-deletion') }}" class="px-3 py-1.5 mx-1 sm:mx-2 hover:text-brand-blue {{ request()->routeIs('legal.data-deletion*') ? 'text-brand-blue' : '' }} transition-colors">{{ __('Data Deletion') }}</a>
    </nav>
    <div class="flex items-center justify-center gap-2 flex-wrap text-center">
        ...
        <span class="px-2 py-0.5 text-xs font-bold text-brand-blue bg-brand-blue/10 border border-brand-blue/20 rounded uppercase tracking-wider">Beta</span>
    </div>
</footer>
