<div class="space-y-4">
        ...
        <div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
            <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-800">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Code') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Target Email') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Status') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Expires') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Created') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @foreach($codes as $code)
                        <tr class="bg-white dark:bg-gray-900 hover:bg-gray-100 dark:hover:bg-gray-800">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $code['email'] ?? __('Anyone') }}</td>
                            <td class="px-4 py-3 text-sm">
                                ...
                                @endphp
                                @if($isUsed)
                                    <x-filament::badge color="gray">{{ __('Used') }}</x-filament::badge>
                                @elseif($isExpired)
                                    <x-filament::badge color="danger">{{ __('Expired') }}</x-filament::badge>
                                @else
                                    <x-filament::badge color="success">{{ __('Available') }}</x-filament::badge>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $code['expires_at'] ? \Carbon\Carbon::parse($code['expires_at'])->format('Y-m-d H:i') : __('Never') }}</td>
    ...
    @else
        <div class="text-center py-8 text-gray-500 dark:text-gray-400">
            {{ __('No share codes generated yet.') }}
        </div>
    @endif
    ...
    @if($showForm)
        ...
        >
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-md mx-4 border border-gray-200 dark:border-gray-700">
                <h4 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">{{ __('Generate Share Code') }}</h4>

                <form wire:submit="generate">
                    <div class="mb-4">
                        <label for="email" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Target Email (optional)') }}</label>
                        <input
                            type="email"
                            id="email"
                            wire:model="email"
                        ...
                        @enderror
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ __('Leave empty to allow anyone to use this code.') }}</p>
                    </div>
