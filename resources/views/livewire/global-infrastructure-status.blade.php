...
<div class="flex-1 flex items-center justify-center gap-4 px-2" wire:poll.30s>
    <div class="flex items-center gap-1.5 px-2 py-1 bg-gray-50 dark:bg-white/5 rounded-lg" title="{{ $statusText }}">
        ...
        <span class="topbar-status-text hidden sm:inline text-xs font-medium text-gray-600 dark:text-gray-200 max-w-[120px] truncate">
            {{ $statusText }}
        </span>
    </div>
    ...
    @if($showSync)
        <a href="{{ \App\Filament\App\Pages\DataSync::getUrl() }}"
           ...
           title="{{ __('Sync Progress') }}: {{ $syncPercentage }}%">
            <div class="topbar-sync-progress hidden sm:block w-20 sm:w-28 bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
    ...
    <div x-data="tenantClock({ timezone: '{{ $tenantTimezone }}' })" 
       ...
       title="{{ __('Current Tenant Local Time') }}">
        <svg xmlns="http://www.w3.org/2000/svg" class="topbar-clock-icon hidden sm:block w-3.5 h-3.5 text-primary-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
        </svg>
        <span x-text="timeStr" class="font-bold tracking-tight"></span>
        <span class="topbar-clock-tz hidden sm:inline text-[10px] text-gray-500 dark:text-gray-400 font-semibold uppercase" x-text="tzStr"></span>
    </div>
</div>
