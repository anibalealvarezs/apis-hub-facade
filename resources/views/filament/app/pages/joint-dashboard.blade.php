<x-filament-panels::page>
    ...
    <div id="joint-dashboard-container" class="joint-page" x-data="jointDashboard(Object.assign({
        ...
        <div
            class="joint-header-row py-3 px-3 mb-6 bg-gray-50/98 dark:bg-gray-900/98 backdrop-blur-md border-b border-gray-200 dark:border-white/10 transition-colors">
        ...
        <div class="joint-card">
            <!-- Playbook Section -->
            <div class="mb-6 pb-6 border-b border-gray-200 dark:border-white/10"
                 x-show="getAvailablePlays().length > 0">
                    ...
                    <template x-for="play in getAvailablePlays()" :key="play.id">
                        <button type="button" @click="applyPlay(play)"
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate"
                                 x-text="play.short_desc"></div>
                        </button>
                    </template>
                </div>

                <!-- Theory Explanation Callout -->
                <div x-show="selectedPlay"
                     class="mt-6 p-4 rounded-xl bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 flex items-start gap-3">
                    <x-heroicon-o-light-bulb class="w-5 h-5 text-blue-500 shrink-0 mt-0.5"/>
                    <div>
                        <h4 class="font-bold text-blue-900 dark:text-blue-300 text-sm mb-2"
                            x-text="selectedPlay ? selectedPlay.name : ''"></h4>
                        <div class="text-sm text-blue-800 dark:text-blue-200 space-y-2">
                            <p><strong>{{ __('Theory:') }}</strong> <span
                                    x-text="selectedPlay ? selectedPlay.theory : ''"></span></p>
                            <p><strong>{{ __('Expected Results:') }}</strong> <span
                                    x-text="selectedPlay ? selectedPlay.expected : ''"></span></p>
            ...
            <div class="joint-form-grid mt-6">
                <!-- Curve A -->
                <div class="joint-curve-section curve-a">
                    ...
                    <div class="space-y-3">
                        <div>
                            <label
                                class="text-sm font-semibold text-gray-600 dark:text-gray-400">{{ __('Channel') }}</label>
                        ...
                        <div x-show="curveA.channel">
                            <label
                                class="text-sm font-semibold text-gray-600 dark:text-gray-400">{{ __('Asset / Property') }}</label>
                        ...
                        <div x-show="curveA.channel">
                            <label
                                class="text-sm font-semibold text-gray-600 dark:text-gray-400">{{ __('Metric') }}</label>
                        ...
                        <div x-show="curveA.channel" class="grid grid-cols-2 gap-3 mt-3">
                            <div>
                                <label
                                    class="text-sm font-semibold text-gray-600 dark:text-gray-400">{{ __('Analysis Level') }}</label>
                            ...
                            <div>
                                <label
                                    class="text-sm font-semibold text-gray-600 dark:text-gray-400">{{ __('Lag (Shift)') }}</label>
        ...
        <div class="joint-card joint-charts-card" x-show="chartRendered" x-cloak>
            <div x-show="isLoading"
                ...
                <div class="flex flex-col items-center">
                    ...
                    <span
                        class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ __('Processing Joint Curves & Correlation...') }}</span>
                </div>
            </div>
            ...
            <div class="flex justify-between items-start mb-10">
                ...
                <div x-show="correlation !== null" class="text-right">
                    <div
                        class="text-xs font-semibold text-gray-500 uppercase tracking-widest">{{ __('Pearson Correlation') }}</div>
