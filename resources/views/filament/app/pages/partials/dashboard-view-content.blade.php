        ...
        <template x-if="dashboardDefaults.show_asset_group_selector">
            <div class="relative" x-data="{ open: false, searchAssetGroup: '' }" @click.outside="open = false">
                <button @click="open = !open" type="button"
                        class="bg-white dark:bg-white/5 border border-gray-300 dark:border-white/10 text-gray-950 dark:text-white text-xs rounded-lg focus:ring-primary-500 focus:border-primary-500 flex items-center justify-between px-3 py-2 h-[34px] min-w-[170px]">
                    <span class="truncate font-medium text-gray-700 dark:text-gray-200"
                          x-text="!selectedAssetGroup ? '{{ __('All Assets (no filter)') }}' : (assetGroups[selectedAssetGroup] || '{{ __('Select Asset Group...') }}')"></span>
                ...
                <div x-show="open" x-transition style="display: none; min-width: 240px;"
                    ...
                    <!-- Search Header -->
                    <template x-if="Object.keys(assetGroups).length > 5">
                        <div class="p-2 border-b border-gray-200 dark:border-gray-700">
                            <input type="text" x-model="searchAssetGroup"
                                   ...
                                   placeholder="{{ __('Search asset groups...') }}">
                        </div>
                    </template>

                    <!-- List of Asset Groups -->
                    <div class="p-1.5 flex flex-col gap-1 overflow-y-auto max-h-60">
                        <div @click="selectedAssetGroup = ''; applyAssetGroup(); open = false;"
                             ...
                             :class="!selectedAssetGroup ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-200 dark:border-primary-800' : 'hover:bg-gray-100 dark:hover:bg-gray-700 border-transparent'">
                            <div class="w-4 h-4 shrink-0 flex items-center justify-center rounded-full border-2 transition-colors duration-150"
                                 :class="!selectedAssetGroup ? 'bg-primary-600 border-primary-600' : 'border-gray-300 dark:border-gray-600'">
                                <svg x-show="!selectedAssetGroup" class="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/>
                                </svg>
                            </div>
                            <span class="truncate font-medium text-gray-900 dark:text-white" :class="!selectedAssetGroup ? 'text-primary-700 dark:text-primary-300 font-semibold' : ''">{{ __('All Assets (no filter)') }}</span>
                        </div>
                        <template x-for="(name, id) in assetGroups" :key="id">
                            <div x-show="searchAssetGroup === '' || name.toLowerCase().includes(searchAssetGroup.toLowerCase())"
                                 @click="selectedAssetGroup = id; applyAssetGroup(); open = false;"
                                 class="flex gap-x-2.5 items-center px-3 py-2 text-xs text-gray-700 dark:text-gray-300 rounded-md cursor-pointer transition-all duration-150 border"
                                 :class="selectedAssetGroup == id ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-200 dark:border-primary-800' : 'hover:bg-gray-100 dark:hover:bg-gray-700 border-transparent'">
                                <div class="w-4 h-4 shrink-0 flex items-center justify-center rounded-full border-2 transition-colors duration-150"
                                     :class="selectedAssetGroup == id ? 'bg-primary-600 border-primary-600' : 'border-gray-300 dark:border-gray-600'">
                                    <svg x-show="selectedAssetGroup == id" class="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/>
                                    </svg>
                                </div>
                                <span class="truncate font-medium text-gray-900 dark:text-white" :class="selectedAssetGroup == id ? 'text-primary-700 dark:text-primary-300 font-semibold' : ''" x-text="name"></span>
                            </div>
                        </template>
                    </div>
                </div>
