...
<div x-data="dashboardView({
    ...
    <div id="view-grid-stack" class="grid-stack relative" wire:ignore>
        @php
            $sectionsList = !empty($viewObj->sections) ? $viewObj->sections : [
                ['index' => 0, 'grid_y_start' => 0, 'grid_y_end' => 9999, 'height' => 9999]
            ];
            $widgetsBySection = collect($viewObj->widgets)->groupBy('section_index');
        @endphp

        @foreach ($sectionsList as $sec)
            <div class="dashboard-print-section" data-section-index="{{ $sec['index'] }}">
                @foreach ($widgetsBySection->get($sec['index'], []) as $wIndex => $widget)
            ...
            @if (!$loop->last)
                <div class="dashboard-print-page-break" data-break-after-section="{{ $sec['index'] }}"></div>
            @endif
        @endforeach
    </div>
