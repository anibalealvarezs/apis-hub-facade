                ...
                <div class="relative" x-data="{ open: false, searchAccount: '' }">
                    ...
                    <div x-show="open" x-transition style="display: none; min-width: 320px;"
                        ...
                        <div class="p-2 flex flex-col gap-1 overflow-y-auto max-h-96">
                            @if(count($accounts) === 0)
                                <div
                                    class="px-3 py-2 text-sm text-gray-500 dark:text-gray-400 italic">{{ __('No accounts available.') }}</div>
                            @endif
                            @foreach($accounts as $id => $name)
                                <div
                                    ...
                                    @click="accounts.includes('{{ $id }}') ? accounts = accounts.filter(a => a !== '{{ $id }}') : accounts.push('{{ $id }}')"
                                    class="flex items-center px-3 py-2 text-sm text-gray-700 dark:text-gray-300 rounded-md cursor-pointer transition-all duration-150 border"
                                    :class="accounts.includes('{{ $id }}') ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-200 dark:border-primary-800' : 'hover:bg-gray-100 dark:hover:bg-gray-700 border-transparent'">
                                    <div class="w-5 h-5 mr-3 shrink-0 flex items-center justify-center rounded-full border-2 transition-colors duration-150"
                                         :class="accounts.includes('{{ $id }}') ? 'bg-primary-600 border-primary-600' : 'border-gray-300 dark:border-gray-600'">
                                        <svg x-show="accounts.includes('{{ $id }}')" class="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" />
                                        </svg>
                                    </div>
                                    <div class="flex flex-col overflow-hidden">
                                        <span class="truncate font-medium" :class="accounts.includes('{{ $id }}') ? 'text-primary-700 dark:text-primary-300' : ''" title="{{ $name }}">{{ $name }}</span>
                                        <span class="text-xs truncate" :class="accounts.includes('{{ $id }}') ? 'text-primary-500 dark:text-primary-400' : 'text-gray-500 dark:text-gray-400'">{{ $id }}</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
