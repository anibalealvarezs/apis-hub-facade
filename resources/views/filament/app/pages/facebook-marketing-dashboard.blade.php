                ...
                <div class="relative" x-data="{ open: false, searchAccount: '' }">
                    <button @click="open = !open" @click.outside="open = false" type="button"
                        ...
                        <span class="truncate font-medium text-gray-700 dark:text-gray-200"
                              x-text="accounts.length === 0 ? '{{ __('Select Ad Accounts...') }}' : (accounts.length === 1 ? '1 {{ __('account') }}' : accounts.length + ' {{ __('accounts') }}')"></span>
                        ...
                        <div class="p-2 flex flex-col gap-1 overflow-y-auto max-h-96">
                            ...
                            @foreach($accounts as $id => $name)
                                <label
                                    ...
                                    class="flex items-center px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md cursor-pointer transition-colors duration-150">
                                    <input type="checkbox" value="{{ $id }}" x-model="accounts"
