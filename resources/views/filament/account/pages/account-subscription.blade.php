<x-filament-panels::page>
    ...
    @if($profile)
        ...
        <div class="mb-8 p-6 bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl shadow-md text-white flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <span class="text-[10px] font-extrabold uppercase tracking-widest bg-white/20 text-white px-2.5 py-1 rounded-full">{{ __('Active Profile') }}</span>
                <h3 class="text-2xl font-bold mt-2">{{ $profile->display_name }}</h3>
                <p class="text-xs text-white/80 mt-1">{{ __('Billing Status:') }} <span class="font-bold uppercase tracking-wider {{ $profile->status === 'active' ? 'text-green-300' : 'text-yellow-300' }}">{{ $profile->status ?? 'Active' }}</span></p>
            </div>
        ...
        <div class="flex justify-center mb-8">
            <div class="bg-gray-100 dark:bg-gray-900 p-1 rounded-xl flex items-center shadow-inner border border-gray-200 dark:border-gray-800">
                ...
                <button @click="billingCycle = 'annual'" 
                    ...
                    Annual
                    <span class="bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300 text-[10px] px-2 py-0.5 rounded-full font-extrabold tracking-wider">{{ __('2 Months Free') }}</span>
                </button>
            </div>
        </div>
