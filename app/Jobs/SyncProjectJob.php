...
class SyncProjectJob implements ShouldQueue
{
    ...
    {
        // Mark the project as syncing immediately so the UI can reflect this state
        $this->project->update([
        ...
        $deploymentLog = ProjectDeploymentLog::create([
            ...
            'status'     => 'running',
            'started_at' => now(),
            'output'     => 'Starting lightweight synchronization process...',
        ]);

        try {
            // 2. Run Sync
            $result = $deployer->startSync($this->project);
