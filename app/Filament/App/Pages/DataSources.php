<?php
    ...
    class DataSources extends Page
    {
        ...
        protected function buildAssetRepeater(string $fieldKey, string $label, array $itemSchema): \Filament\Forms\Components\Component
        {
            ...
            ])->extraAttributes(['x-data' => "{ assetFilter: '', assetStatusFilter: 'all', assetGraceFilter: '', assetGroupFilter: 'all', statusOptions: { 'all': '".__('All Statuses')."', 'enabled': '".__('Enabled Only')."', 'disabled': '".__('Disabled Only')."' }, graceOptions: { '': '".__('Asset Billing Status')."', 'all': '".__('All States')."', 'grace': '".__('In Grace Period')."', 'locked': '".__('Asset Locked')."', 'none': '".__('No Grace Period status')."' }, get groupOptions() { const opts = { 'all': '".__('All Groups')."' }; if (typeof assetGroupsData !== 'undefined' && assetGroupsData && assetGroupsData.groups) { assetGroupsData.groups.forEach(g => { opts[g.name] = g.name; }); } return opts; }, sortAssets() { let ul = this.\$root.querySelector('ul'); if (ul) { let items = Array.from(ul.children); items.sort((a, b) => { let textA = a.innerText.trim().split('\\n')[0]; let textB = b.innerText.trim().split('\\n')[0]; return textA.localeCompare(textB, undefined, {sensitivity: 'base'}); }); items.forEach(li => ul.appendChild(li)); } }, init() { this.\$watch('activeTab', value => { this.assetFilter = ''; this.assetStatusFilter = 'all'; this.assetGraceFilter = ''; this.assetGroupFilter = 'all'; }); } }", 'class' => 'w-full']);
        }
