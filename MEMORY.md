...
- **Override:** Filament's `filament-panels` view namespace only resolves to the vendor path (spatie `bootPackageViews` never registers `resources/views/vendor/filament-panels`). Registered the override manually in `app/Providers/Filament/AppPanelProvider.php::boot()`: `$this->app['view']->getFinder()->prependNamespace('filament-panels', resource_path('views/vendor/filament-panels'));`. New file `resources/views/vendor/filament-panels/components/page/sub-navigation/sidebar.blade.php` mirrors the vendor component with:
  ...
  - `<ul>` gained `x-show="$store.subnav.isOpen" x-cloak` (kept `wire:ignore`); both render hooks preserved.
...
- **CSS** (`public/css/filament-extras.css`): `.fi-page-sub-navigation-sidebar-ctn { width: 18rem; transition: width 0.2s ease; }`, `.fi-page-sub-navigation-sidebar-ctn.fi-subnav-closed { width: 2.25rem; }`, `.fi-subnav-toggle` (+ hover + `.dark` hover).
