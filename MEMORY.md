...
- **Implementation**:
  ...
  - **High-Contrast Typography, Dynamic Theming & Layout**:
...
- **Verification:** `node --check` on all 3 JS files, `php -l`, `npm run build` succeeded.

### Cloudflare-Interrupted Widget Data Requests: Auto-Retry Flow (2026-09-23)
- **Problem:** When a widget data request hits a Cloudflare interruption/gateway timeout (SaaS engine side), `RemoteEngineService::execute()` collapsed the SDK/guzzle exception into `['status'=>'error','message'=>...]`. Derived metric / KPI handlers threw `RuntimeException` → `show()` catch returned a static red error state; metric/entity handlers swallowed the error to empty data. User wanted a structured, retryable signal + a per-widget auto-reload warning and a 2-minute retry loop.
- **Detection (structured signal, not message-only regex — `sanitizeErrorMessage` rewrites cURL/Connection texts):**
  - `RemoteEngineService::execute()` catch now classifies the engine error message and, when retryable, adds `'retryable' => true` + `'error_code' => 'cloudflare_timeout'` to the returned error array. New `RemoteEngineService::isCloudflareInterruption(string $message): bool` matcher: cURL error 28, Operation timed out, connect() timed out, Connection timed out, timed out after, Request Timeout, Gateway Timeout, Cloudflare, A Timeout Occurred, 504, 502, 520-525.
  - New `app/Exceptions/RetryableEngineException.php` (extends RuntimeException).
  - `DashboardWidgetDataController::handleKpiSource()` (~line 2280): throws `RetryableEngineException` when `$result['retryable']`; else plain `RuntimeException`. `show()` top-level catch (~line 1505) handles `instanceof RetryableEngineException` OR `RemoteEngineService::isCloudflareInterruption($e->getMessage())` → returns `['success'=>false,'retryable'=>true,'error_code'=>'cloudflare_timeout','error'=>'This widget needs a bit more time to load its data. It will automatically reload in a few moments.']` (no `message` key; renderer reads `message||error`).
  - Metric/entity (non-KPI) path: `GoogleSearchConsoleController` and `FacebookMarketingController` now include `'retryable' => !empty($results[...]['retryable'])` in summary/chart/table responses. `forwardToChannelEndpoint()` returns `$data['data'] ?? $data`, so the `retryable` key flows into handler return values. `show()` checks `!empty($data['retryable'] ?? false)` right after the `$data = match(...)` (~line 221, `unset($data['retryable'])` then returns the same retryable response). FBO/GA controllers NOT modified (they don't check `status:error`; renderer message-pattern fallback covers surfaced messages).
- **Renderer (`public/js/dashboard-renderer.js`, direct edit — no build):** on `success:false` + `json.retryable` (or `Error.message` matching `isRetryableError()` patterns), renders amber state `cloudflareTimeoutState(message)` ("Still Loading — Auto-Retrying ... automatically reload in 2 minutes" + Retry Now button) and schedules `_scheduleRetry(widgetId, containerEl, controls, tenant)` → per-widget `_retryTimers` Map, 2-minute `setTimeout` that re-renders via `renderWidget` (guarded by `containerEl.isConnected`); success path calls `_cancelRetry(widgetId)`. "Retry Now" button dispatches bubbled `widget-reload-now` event on `.widget-content` (the container). Backoff in `_enqueue` still retries cURL 28/timeout messages up to 3 attempts before the amber state shows.
- **Per-widget reload icon (manual override):** added a reload-button in the widget header action row of `dashboard-view-content.blade.php` (~line 355, before the settings gear) wired to existing `widgetHeader.updateWidget()` (calls root `reloadWidget`, clears cache + re-renders). No JS bundle change needed.
- **Verification:** `php -l` on all touched PHP files; `node --check` on `dashboard-renderer.js`. Bluprint/lint of facade files not possible from this IDE (outside `apis-hub` root) — used PHP CLI. `RetryableEngineException` first created in wrong root (`apis-hub`) — moved to `apis-hub-facade/app/Exceptions/`. Not yet committed; runtime behavior unverified (no deploy/DB in this IDE).
- **Note (carryover):** `RemoteEngineService.php` also carries an uncommitted 50→60s timeout change from earlier work.
...
- **Fix:**
  ...
  4. `dashboard-builder.blade.php` (rank-by select, ~line 1547): originally used `x-model="series.breakdown.rank_by"` on a native `<select>` whose `<option>`s are generated by Alpine `x-for`. **Bug (2026-09-23):** Alpine evaluates `x-model` on the select node BEFORE expanding the nested `<template x-for>` into options, so `select.value = "clicks"` is applied while no matching option exists and never re-applies — the persisted `rank_by` was correctly present in `widgetControlsForm.raw_series[i].breakdown` (proven via captured Livewire save→reload hydration for widget 115) but the selector visually rendered unselected. **Fix:** switched to the established dimension-select pattern (lines 1496-1506): `x-bind:value` on the select + `x-on:change` writing `series.breakdown.rank_by` + per-option `x-bind:selected="series.breakdown && series.breakdown.rank_by === mKey"`. No JS bundle change needed (blade is server-rendered).
...
- **Backwards compatibility:** `rank_by` is optional JSON on `DashboardWidget::controls`; absent/invalid values fall back to the previous behavior (`$metrics[0]`). Legacy widgets saved before the feature (e.g. 112/116, last saved before commit `568077c5` 13:04:39) have no stored `rank_by` and default to `metrics[0]` on open via `ensureBreakdownRankByMetric`.
- **Verification:** persistence/hydration confirmed from captured Livewire request+response for widget 115 (`breakdown.rank_by: "clicks"` in both); widget 115 metrics `["position","clicks","impressions","ctr"]` include `clicks`, so the non-default choice is preserved. `php -l` and `node --check` pass.
