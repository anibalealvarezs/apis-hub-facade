...
- **CSS** (`public/css/filament-extras.css`): `.fi-page-sub-navigation-sidebar-ctn { width: 18rem; transition: width 0.2s ease; }`, `.fi-page-sub-navigation-sidebar-ctn.fi-subnav-closed { width: 2.25rem; }`, `.fi-subnav-toggle` (+ hover + `.dark` hover), plus collapsed-state rules that keep item/group icons visible (see above).
